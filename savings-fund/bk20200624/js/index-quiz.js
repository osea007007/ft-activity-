...
var front = {
  ...
  data() {
    return {
      config: {
        event_id: "L0008",
        event_code: "savings-fund",
        event_sub: "quiz",
        // discount_code: "",
        start_date: "2020-06-10 18:00:00",
        end_date: "2020-07-01 10:00:00",
        uid: "",
        t: "",
        hash: "",
        isBinded: 999,
  ...
  computed: {
    ...
    event_url() {
      return this.testMode ? "https://line.franklin.com.tw:8443/Tag/Bind/DR?id=savingquiz" : "https://line.franklin.com.tw:8843/Tag/Bind/DR?id=savingquiz";
    },
    wf09Data() {
      var data = {
    ...
    isLine() {
      // 這個活動只做在LINE上
      return this.testMode ? this.getQueryParams().utm_source == "linem" : true;
    },
    isEnd() {
  ...
  methods: {
    ...
    checkAnswer() {
      ...
      this.answer == 1 ? this.next() : this.prev();
    },
    prev() {
      this.isError = true;
      this.$nextTick(() => {
        this.$scrollTo(".diamondBg");
      });
    },
    next() {
      if (this.step == 3) {
        ...
        this.$nextTick(() => {
          this.$scrollTo(".completeQuestionnaire");
        });
        return;
      } else {
        var $this = this;
    ...
    checkLineParams() {
      this.checkParams(this.config.event_code, () => {
        ...
        this.testLog(
          "url",
          `https://wt.franklin.com.tw/areas/${this.config.event_code}/${this.config.event_sub}.html?utm_source=${this.getQueryParams().utm_source}&uid=${
            this.config.uid
          }&t=${this.config.t}&hash=${this.config.hash}`
        );
      });
    },
    async getInfo() {
