  ...
  data: function() {
    return {
      list: [
        {
          id: 0,
          info: `<b class="footer-warning-focus">本基金經金融監督管理委員會核准或申報生效在國內募集及銷售，惟不表示絕無風險。基金經理公司以往之經理績效不保證基金之最低投資收益；基金經理公司除盡善良管理人之注意義務外，不負責本基金之盈虧，亦不保證最低之收益，投資人申購前應詳閱基金公開說明書。</b>`
        },
        {
          id: 1,
          info: `基金過去績效不代表未來績效之保證。`
        },
        {
          id: 2,
        ...
        {
          id: 3,
          info: `<b class="footer-warning-focus">基金的配息可能由基金的收益或本金中支付。任何涉及由本金支出的部份，可能導致原始投資金額減損。部份基金進行配息前未先扣除應負擔之費用。由本金支付配息之相關資料已揭露於本公司網站，投資人可至本公司網站(<a href="https://www.franklin.com.tw" target="_blank">https://www.franklin.com.tw</a>)查閱。基金配息率不代表基金報酬率，且過去配息率不代表未來配息率；基金淨值可能因市場因素而上下波動，投資人於獲配息時，宜一併注意基金淨值之變動。</b>`
        },
        {
          id: 4,
...
// testMode
// ========= testMode ==============================================================================================================================
Vue.component("test-mode", {
  props: ["step"],
  template: `<div class="toggleStage" v-if="testMode">
              <div @click="$emit('test', '2020-02-03')">1</div>
              <div @click="$emit('test', '2019-12-18')">2</div>
              <div @click="$emit('line')">綁LINE是客戶</div>
            </div>`,
  data() {
    return {};
  },
  computed: {
    testMode() {
      return location.hostname != "event.franklin.com.tw" ? true : false;
    }
  }
});
...
var content = new Vue({
  ...
  mounted() {
    ...
    // this.showTarget();
    // this.scrollMagic();
    this.sameHeight("fundItem-bottom");
  ...
  methods: {
    ...
    setLatestOffersUrl() {
      return (this.latestOffersUrl =
        this.windowWidth >= "992" ? "https://www.franklin.com.tw/ReservationAccount/ECOffer" : "https://www.franklin.com.tw/Mobile/Activity/ActivityOffer");
    },
    ...
    scrollMagic() {
      ...
      new ScrollMagic.Scene({ triggerElement: ".banner", offset: 0 }).setTween(bannerTimeLine).addTo(controller);

      bannerTimeLine.add(
        TweenMax.from(".banner-text", 0.5, {
          y: 100,
          opacity: 0
        })
      );
